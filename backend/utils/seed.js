...
async function seed() {
  ...
  console.log("[seed] Clearing sample issues...");
  await Issue.deleteMany({});

  console.log("[seed] Removing demo accounts...");
  await User.deleteMany({
    email: {
      $in: [
        "roads.authority@strix.demo",
        "sanitation.authority@strix.demo",
        "citizen@strix.demo",
      ],
    },
  });

  console.log("[seed] Creating your admin account...");

  const adminEmail = process.env.ADMIN_EMAIL;
  const adminPassword = process.env.ADMIN_PASSWORD;
  const adminName = process.env.ADMIN_NAME || "Strix Administrator";
